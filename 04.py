...
PASSPORTS = """eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
...
def validate_hgt(passport):
    hgt_value, hgt_unit = int(passport['hgt'][:-2]), passport['hgt'][-2:]
...
def validate_hcl(passport):
    hcl = passport['hcl']
    result = match('#[0-9a-f]{6}', hcl)
    if len(hcl) != 7 or not bool(result):
        raise Exception('hcl')
...
def validate_pid(passport):
    pid = passport['pid']
    result = match('[0-9]{9}', pid)
    if len(pid) != 9 or not bool(result):
        raise Exception('pid')
...
if __name__ == '__main__':
    ...
    passport_list = parse_passports()

    compulsory_fields = set(VALIDATION_MAP.keys())

    # Part 1.
    complete_passport_list = [
        passport for passport in passport_list
        if not len(compulsory_fields - set(passport.keys()))
    ]
    count = len(complete_passport_list)
