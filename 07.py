...
BAGS = """pale turquoise bags contain 3 muted cyan bags, 5 striped teal bags.
...
def parse_bags():
    ...
    for row in rows:
        reduced = row.split(' bag')
        primary = reduced[0]
        if primary in bags.keys():
            print(primary)
        for pair in reduced[1:]:
            pair = pair.lstrip()
            if pair == '':
                pass
            elif pair == 'no other':
                ...
                i = pair.index(' ')
                n = int(pair[0:i])
                colour = pair[i:].lstrip()
                bags[primary][colour] = n
    return bags


def dfs1(connectivity, node, target, visited):

    child_nodes = set(connectivity[node]) - set(visited)
    ...
    for node in child_nodes:
        if node not in visited_copy:
            ...
            dfs1(connectivity, node, target, visited_copy)


def dfs2(connectivity, node, parent_multiplier, visited, count):
    for child_node, n in connectivity[node].items():
        multiplier = n * parent_multiplier
        count[0] += multiplier
        dfs2(connectivity, child_node, multiplier, visited, count)
